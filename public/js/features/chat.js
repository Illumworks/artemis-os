...
import { addUserMessage, appendAssistantText, appendToolIndicator, appendToolResult, showThinking, removeThinking, addResultSummary, addStatus, showChatEmptyState, addSkillUsedMessage } from '../ui/messages.js';
...
function historyKey() {
  return "artemis-input-history-" + ($.projectSelect?.value || "default");
}
...
function showWaitingForInput(pane) {
  ...
  const inputBar = parallelMode
    ? pane.messageInput?.closest('.pane-composer, .input-bar')
    : $.messageInput?.closest('.dp-composer, .input-bar');
...
function formatServerError(msg) {
  if (msg?.code === "error_codex_resume") {
    return "Codex could not safely continue this conversation. Start a new Codex session to keep going.";
  }
  return "Error: " + (msg?.error || "Unknown error");
}

function generatePendingSessionId() {
  if (globalThis.crypto?.randomUUID) {
    return globalThis.crypto.randomUUID();
  }
  return `artemis-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}

function ensureParallelSessionId() {
  if (!getState("parallelMode")) return getState("sessionId");
  const existing = getState("sessionId");
  if (existing) return existing;
  const pendingId = generatePendingSessionId();
  setState("sessionId", pendingId);
  return pendingId;
}

const ACTIVITY_STAGE_DELAYS_MS = {
  connecting: 4000,
  thinking: 10000,
};

function getProviderActivityName(provider) {
...
function getActivityLabel(provider, stage) {
  const name = getProviderActivityName(provider);
  if (stage === "connecting") return `Connecting to ${name}...`;
  if (stage === "thinking") return `${name} is thinking...`;
  if (stage === "working") return `${name} is still working...`;
  return "Thinking...";
}

function clearProviderActivity(pane) {
  if (pane?._activityTimer) {
    clearTimeout(pane._activityTimer);
    pane._activityTimer = null;
  }
}

function getComposerPlaceholder() {
  return "Ask a question, plan a task, or start a repo change...";
}

function updateComposerState(pane = getPane(null)) {
  const parallelMode = getState("parallelMode");
  const input = parallelMode ? pane?.messageInput : $.messageInput;
  const sendBtn = parallelMode ? pane?.sendBtn : $.sendBtn;
  if (!input || !sendBtn) return;

  const hasProject = Boolean($.projectSelect?.value);
  const hasText = Boolean(input.value?.trim());
  const isStreaming = Boolean(pane?.isStreaming);

  input.placeholder = getComposerPlaceholder();
  sendBtn.disabled = !hasProject || !hasText || isStreaming;
}
...
function consumeShellIntent() {
  ...
  const intent = localStorage.getItem(SHELL_INTENT_STORAGE_KEY);
  if (!intent) return;
  ...
  if (composerContextNote) {
    composerContextNote.textContent = localStorage.getItem(COMPOSER_CONTEXT_NOTE_STORAGE_KEY)
      || "Dashboard handoff loaded into Chat.";
  }
  updateComposerState();
  localStorage.removeItem(SHELL_INTENT_STORAGE_KEY);
  localStorage.removeItem(COMPOSER_CONTEXT_NOTE_STORAGE_KEY);
}

function setProviderActivity(pane, stage = "connecting") {
  pane = pane || getPane(null);
  const provider = pane._activeProvider || getSelectedProvider() || "claude-code";
  clearProviderActivity(pane);
  showThinking(getActivityLabel(provider, stage), pane);

  if (!pane.isStreaming) return;

  if (stage === "connecting") {
    pane._activityTimer = setTimeout(() => {
      if (!pane.isStreaming) return;
      setProviderActivity(pane, "thinking");
    }, ACTIVITY_STAGE_DELAYS_MS.connecting);
    return;
  }
  ...
  if (stage === "thinking") {
    pane._activityTimer = setTimeout(() => {
      if (!pane.isStreaming) return;
      setProviderActivity(pane, "working");
    }, ACTIVITY_STAGE_DELAYS_MS.thinking);
  }
}
...
export async function sendMessage(pane) {
  ...
  if (text.startsWith("/")) {
    ...
    if (match) {
      ...
      // Not a registered command (or a skill entry) — try as a skill slug
      const skill = await api.fetchSkillBySlug(cmdName);
      if (skill && skill.status === "approved") {
        _skillMatch = { skill, extra: args.trim() };
      }
    }
  }

  // Prepend attached files (or skill body for slash-skill invocations)
  let fullMessage;
  const attachedFiles = getState("attachedFiles");
  if (_skillMatch) {
    const { skill, extra } = _skillMatch;
    fullMessage = `[Skill: ${skill.name}]\n\n${skill.body || ""}`;
    if (extra) fullMessage += `\n\n---\n${extra}`;
  } else {
    fullMessage = text;
  ...
  pane._reloadToken = (pane._reloadToken || 0) + 1;
  addUserMessage(text, pane, images, filePaths);
  if (_skillMatch) {
    addSkillUsedMessage(_skillMatch.skill.name, _skillMatch.skill.description || "", pane);
  }
  inputHistory.add(text);
  inputHistory.reset();
  updateHistoryButtonVisibility();
  ...
  pane._activeProvider = provider || "claude-code";
  const sessionId = ensureParallelSessionId() || getState("sessionId");
  pane._streamSessionId = sessionId;
  const payload = {
    type: "chat",
    message: fullMessage,
    cwd,
  ...
  setProviderActivity(pane, "connecting");
}
...
export function finishStreamingHandler(pane) {
  ...
  // REMOVED: Post-stream DB reload that was causing messages to disappear
  // Fork buttons are now added during streaming (see appendAssistantText)
  // This also reduces unnecessary DB queries after each response
}
...
onState("view", (view) => {
  if (view !== "chat") return;
  queueMicrotask(() => consumeShellIntent());
});

queueMicrotask(() => {
  if (getState("view") === "chat") {
    consumeShellIntent();
  }
});
...
function handleServerMessage(msg) {
  ...
  if (
    ...
    if (
      pane?.isStreaming &&
      pane?._streamSessionId &&
      msg.sessionId === pane._streamSessionId &&
      (msg.type === "done" || msg.type === "error" || msg.type === "aborted")
    ) {
  ...
  switch (msg.type) {
    ...
    case "error":
      finishStreamingHandler(pane);
      addStatus(formatServerError(msg), true, pane);
      break;
...
function showWorktreeConfirmation(ws, payload, pane) {
  ...
  function send(useWorktree) {
    ...
    ws.send(JSON.stringify(payload));
    const provider = getSelectedProvider();
    const connectLabel = provider === "codex" ? "Connecting to Codex..." : provider === "local" ? "Connecting to Local..." : provider === "gemini" ? "Connecting to Gemini..." : provider === "openrouter" ? "Connecting to OpenRouter..." : "Connecting to Claude...";
    showThinking(useWorktree ? "Creating worktree..." : connectLabel, pane);
  }
...
on("ws:reconnected", async () => {
  ...
  try {
    ...
    // 2. Fix (2026-04-19, Claude/Sonnet): if any pane was actively streaming,
    // do NOT reload messages from DB — the stream is still live server-side
    // (ws-handler no longer breaks on disconnect) and the server will send a
    // "done"/"result" message when it finishes. Reloading here replaced the
    // in-progress DOM content with a stale DB snapshot, causing messages to
    // disappear mid-stream. Only reload if nothing was streaming.
    const wasStreaming = [...panes.values()].some(p => p.isStreaming);
...
// Populate commandRegistry with approved skills from the server
export async function loadSkillCommands() {
  try {
    const skills = await api.fetchSkills({ status: "approved" });
    if (!Array.isArray(skills)) return;
    for (const [name, cmd] of Object.entries(commandRegistry)) {
      if (cmd.isSkill) delete commandRegistry[name];
    }
    for (const skill of skills) {
      if (!skill.slug) continue;
      registerCommand(skill.slug, {
        category: "skill",
    ...
    // silently ignore — skills won't appear in autocomplete
  }
}
