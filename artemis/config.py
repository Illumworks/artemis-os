...
class Settings(BaseSettings):
    ...
    argus_board_minutes_body_cap: int = Field(
        ...
        description=(
            ...
            "items whose titles pass artemis.argus.board_relevance."
            "is_argus_relevant (ARGUS-4 — replaced the board-minutes scout's "
            "own, much broader mapping._is_relevant here; see that module's "
            "docstring). Default 20 — measured against live Dallas ISD data "
            "on 2026-08-13, 146 agenda items produced 16 relevant titles "
            "under the old shared predicate, ALL of them false positives for "
            "literacy-procurement purposes; under is_argus_relevant, 0 of "
            "146 pass. This bounds HTTP VOLUME (extra requests against "
            "BoardDocs); argus_board_minutes_body_budget_s separately bounds "
            "WALL-CLOCK TIME. A district with more relevant items than this "
            "cap gets the first `cap` bodies fetched and the rest returned "
            "title-only rather than skipped outright."
        ),
    )
