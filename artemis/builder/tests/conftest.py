"""Test fixtures for artemis.builder tests.

Requires a running Postgres at ARTEMIS_TEST_DB_URL (or ARTEMIS_DB_URL),
already migrated via `alembic upgrade head`.

Mirrors the pattern from artemis/builders/tests/conftest.py:
- NullPool for fresh connections per test
- attach_pgvector_codec defensive call
- TRUNCATE builder + trajectory tables before each test, RESTART IDENTITY CASCADE
"""
