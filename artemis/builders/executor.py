...
def default_agent_instruction(agent_id: str, override: str | None = None) -> str:
    ...
    if agent_id.startswith("marketing.scout."):
        return (
            ...
            "not read. A publisher's homepage is NOT the item's URL and is "
            "rejected as one -- when a link is a long opaque redirect you cannot "
            "open, that redirect is still the right answer.\n\n"
            "PASS THROUGH WHAT THE TOOL GAVE YOU. If an item carried a publisher "
            "and a publisher domain, write them as sourcePublisher and "
            "sourceDomain; if it carried its own headline, write that as "
            "sourceTitle. Dropping them costs the reader the only provenance they "
            "have for a link that cannot be opened directly.\n\n"
