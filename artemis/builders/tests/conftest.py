"""Test fixtures for Builders tests.
...
Mirrors the pattern from artemis/marketing/tests/conftest.py:
- NullPool so each test gets fresh connections, no pool-cached state
- attach_pgvector_codec defensive call
- TRUNCATE the builders tables before each test, RESTART IDENTITY CASCADE
"""
