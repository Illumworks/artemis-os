...
3. **claude-code loop limits — RESOLVED 2026-05-27 (CLI reality check):** `--max-turns` does NOT exist in the installed claude CLI; `--max-budget-usd` has unclear semantics on subscription auth. **The runaway guard is the existing 300s wall-clock subprocess timeout in the adapter** (make it env-configurable: `ARTEMIS_CLAUDE_CODE_TIMEOUT_SECONDS`, default 300). On subscription there's no per-token $ cost, so wall-clock is the correct bound. Also use `--strict-mcp-config` so only our artemis MCP server loads, and `--permission-mode` set for autonomous headless tool calls (CC2 verifies the exact value).
