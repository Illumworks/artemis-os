...
## ✅ Screen-Time go-live — LIVE (main 3d13dd9, decoupled per Jon; NO auto-push)
Owner decision honored: collect + report-on-demand now; ZERO auto-push. All shipped:
1. **Collection cron** — daily 11:00 UTC, wired into main.py, COLLECTION ONLY. Fixed the LegiScan status bug (vetoed/failed ≠ "passed") + the display-only pipeline landmine. **CRITICAL FIX:** `report_channel` was actually SET (C0BBYM8N26M), so the cron *would* have auto-broadcast — added `deliver_alerts=False` on the cron path so it collects SILENTLY regardless.
2. **Board coverage** — 27 priority-state districts (josh_spec states, all BoardDocs-verified), general-intel mode. Runs via the SILENT screen-time fan-out only; the standalone scout scheduler stays DISABLED (it would feed the marketing signal_queue → Callie auto-push). Non-customer exclusion is a plug-in for when the customer list lands.
3. **Callie on-demand report** — `get_screentime_report` (callie-only, read-only): Amy/others just chat with Callie; NO auto-broadcast. `record_screentime_feedback` wires deny-with-reason into the existing callie_push reaction-learning (reasoned-reject teaches; silent ignore = nothing).

**Data note:** current DB is stale (17 signals/10 states, pre-broadening) until the first cron sweep runs (11:00 UTC) or a manual sweep is triggered — then the 50-state + AI + board coverage lights up.
**Held for Jon/Angela:** (a) STANCE tuning (favorable/unfavorable; chatbot ban ≠ bad for Amira) — tune against real data; (b) optional "worth a look" light announce mode (deferred, keeps noise low); (c) the non-customer exclusion (Salesforce/Product list, still stalled).
