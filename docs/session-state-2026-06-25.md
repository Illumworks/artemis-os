...
News outlets (Chalkbeat/EdSource/K-12 Dive/EdWeek/GovTech/The 74/Hechinger/Axios) + keywords; state-DOE 7→20 states; board scout wired into the fan-out with a 13-district live-verified seed. Board scout kept **DISABLED** (Salesforce exclusion pending — enabling would flag customers as "peer validation").
  - **DECISION MADE (Jon, 2026-07-10): widen the topic gate to include AI-in-schools policy** — "screen time and AI policy, they're related." IN PROGRESS on branch `lead/screentime-ai-topic`: add AI-policy anchors to topic_config.py require_any + the LegiScan fanout terms (multi-word anchors only, never bare "ai"). AI-policy stance nuance (bans on open chatbots are NOT unfavorable to Amira — it's in the standards-aligned carve-out) DEFERRED to the Angela stance review.
