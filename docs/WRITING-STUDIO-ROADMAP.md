...
| 3 | **Tagging taxonomy + tag-scoped rules engine** — ✅ **Phase 1 (registry) BUILT+MERGED** (Codex, migration 0070, `/api/writing-studio/tags`, 5 dims/41 values, lossless) — `briefs/codex-ws-phase-1-tag-registry.md`. Remaining: rules engine (tag-scoped) + tagging UI + AI auto-tagging + composer integration | `writing-studio-tagging-and-rules-engine.md` |
