...
async def test_exact_email(db_session: AsyncSession) -> None:
    await _seed(
        ...
        [
            _person(
                email="angela@amiralearning.com",
                full_name="Angela Smith",
                first_name="Angela",
                last_name="Smith",
            )
        ],
...
async def test_exact_full_name(db_session: AsyncSession) -> None:
    await _seed(
        ...
        [
            _person(
                email="greg@istation.com",
                full_name="Greg Shrader",
                first_name="Greg",
                last_name="Shrader",
            )
        ],
...
async def test_first_last(db_session: AsyncSession) -> None:
    await _seed(
        ...
        [
            _person(
                email="kristen.j@amiralearning.com",
                full_name="Kristen Jones",
                first_name="Kristen",
                last_name="Jones",
            ),
            _person(
                email="bob@amiralearning.com",
                full_name="Bob Lee",
                first_name="Bob",
                last_name="Lee",
            ),
...
async def test_first_initial_single_resolves(db_session: AsyncSession) -> None:
    ...
    await _seed(
        ...
        [
            _person(
                email="julie.k@amiralearning.com",
                full_name="Julie Kim",
                first_name="Julie",
                last_name="Kim",
            ),
            _person(
                email="mark@amiralearning.com",
...
async def test_first_initial_ambiguous_returns_none(db_session: AsyncSession) -> None:
    ...
    await _seed(
        ...
        [
            _person(
                email="julie.kim@amiralearning.com",
                full_name="Julie Kim",
                first_name="Julie",
                last_name="Kim",
            ),
            _person(
                email="julie.knox@amiralearning.com",
                full_name="Julie Knox",
                first_name="Julie",
                last_name="Knox",
            ),
        ],
...
async def test_first_name_only_ambiguous(db_session: AsyncSession) -> None:
    await _seed(
        ...
        [
            _person(
                email="julie.a@amiralearning.com",
                full_name="Julie Adams",
                first_name="Julie",
                last_name="Adams",
            ),
            _person(
                email="julie.b@amiralearning.com",
                full_name="Julie Brown",
                first_name="Julie",
                last_name="Brown",
            ),
        ],
...
async def test_first_name_only_unique_resolves(db_session: AsyncSession) -> None:
    ...
    await _seed(
        ...
        [
            _person(
                email="angela.miata@amiralearning.com",
                full_name="Angela Miata",
                first_name="Angela",
                last_name="Miata",
            ),
            _person(
...
async def test_exact_first_name_beats_fuzzy(db_session: AsyncSession) -> None:
    ...
    await _seed(
        ...
        [
            ...
            _person(
                email="angel.blandero@amiralearning.com",
                full_name="Angel Blandero",
                first_name="Angel",
                last_name="Blandero",
            ),
            _person(
                email="mark.angel@amiralearning.com",
                full_name="Mark Angel",
                first_name="Mark",
                last_name="Angel",
            ),
        ],
...
async def test_fuzzy_match(db_session: AsyncSession) -> None:
    await _seed(
        ...
        [
            _person(
                email="kristen@amiralearning.com",
                full_name="Kristen Jameson",
                first_name="Kristen",
                last_name="Jameson",
            )
        ],
...
def _members_v1() -> list[dict]:
    return [
        {
            "id": "U1",
            "real_name": "Angela Smith",
            "profile": {"email": "Angela@amiralearning.com", "display_name": "Ang"},
        },
        {"id": "U2", "real_name": "Greg Shrader", "profile": {"email": "greg@istation.com"}},
        {
            "id": "U3",
            "real_name": "Botty",
            "is_bot": True,
...
async def test_sync_upserts(db_session: AsyncSession, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("artemis.directory.sync._load_working_slack_client", _fake_loader)
    ...
    rows = (
        await db_session.execute(
            text("SELECT email, first_name, last_name FROM directory_people ORDER BY email")
        )
    ).all()
...
async def test_sync_rerun_no_dupes(
